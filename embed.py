...
class TrajectoryEmbedder(Embedder, relabel.RewardLabeler):
  ...
  def _compute_losses(
    ...
    # compute L2 distance between trajectory embedding and task encoding
    a = id_contexts.unsqueeze(1)
    b = torch.detach(a.expand(all_transition_contexts.shape))

    transition_context_loss = (b - all_transition_contexts)**2
    transition_context_loss = torch.sum(transition_context_loss, -1)
  ...
  def label_rewards(self, trajectories):
    ...
    a = torch.detach(id_contexts.unsqueeze(1).expand(all_transition_contexts.shape))
    b = (a - all_transition_contexts)**2
    distances = torch.sum(b, -1) - self._penalty
...
class InstructionPolicyEmbedder(Embedder):
  ...
  def forward(self, states, hidden_state):
    ...
    observ_embed = self._obs_embedder([state.observation for state in states])
    instr_embed = self._instruction_embedder([torch.tensor(state.instructions) for state in states])
    traj_embed = self._trajectory_embedder([state.trajectory for state in states])

    aux_losses = traj_embed[1]

    #pdb.set_trace()
    embeddings = [observ_embed, instr_embed, traj_embed[0]]

    input_embeddings = torch.cat(embeddings, -1)
